...
class BinaryRouter:
    ...
    def _heuristic_p_raw(self, evidence: EvidenceSet, feats: List[float]) -> float:
        ...
        # Lab / clinical indicator keywords → exact values needed from raw text
        _LAB_KEYWORDS = [
            "wbc", "rbc", "hemoglobin", "platelet", "creatinine", "glucose",
            "cholesterol", "triglyceride", "alt", "ast", "bilirubin",
            "sodium", "potassium", "calcium", "white blood cell", "red blood cell",
            "blood pressure", "heart rate", "hba1c", "a1c",
        ]
        # Completeness enumeration → summaries risk truncation
        _COMPLETENESS_KEYWORDS = [
            "all symptoms", "every symptom", "all side effects", "every side effect",
            "list all", "complete list", "all effects", "every effect",
            "what are all", "across all visits", "throughout all", "in total across",
        ]
        # Cross-visit / temporal recall → need original conversation text
        _CROSS_VISIT_KEYWORDS = [
            "previously mentioned", "first reported", "mentioned before",
            "last visit", "first visit", "earlier visit", "prior visit",
            "i said", "i told", "i reported", "i mentioned",
        ]

        if any(k in q for k in _LAB_KEYWORDS):
            score += 0.30
        if any(k in q for k in _COMPLETENESS_KEYWORDS):
            score += 0.30
        if any(k in q for k in _CROSS_VISIT_KEYWORDS):
            score += 0.25
...
class LLMRouter:
    ...
    def decide(
        ...
        Args:
            ...
            domain_addendum: 可选的领域特定规则文本，追加到 prompt 末尾（如 MEDICAL_ROUTER_ADDENDUM）
