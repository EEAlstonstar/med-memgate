...
# === Medical Domain Router Addendum ===
# Append to ROUTER_PROMPT_V2 (or LLMRouter prompt) when handling medical queries.
# Signals that medical queries often require R-path for precision.

MEDICAL_ROUTER_ADDENDUM = """
### Medical Domain Routing Rules:

Force R if the query involves ANY of the following:
- Laboratory / clinical indicators (e.g., "WBC count", "glucose level", "creatinine",
  "platelet", "hemoglobin", "ALT", "AST", "bilirubin", "sodium", "potassium")
- Completeness requirement ("all symptoms", "every side effect", "list all",
  "complete list", "what are all")
- Cross-visit recall ("previously mentioned", "first reported", "mentioned before",
  "last visit", "first visit", "earlier visit", "prior visit",
  "I said", "I told", "I reported")
- Exact drug / compound identifiers (proper-noun chemical names, all-caps abbreviations)
- Exact dosage or measurement queries ("how many mg", "what dose", "exact amount")
- Multi-visit enumeration ("across all visits", "throughout all consultations",
  "in total across")

S is appropriate ONLY if the query asks for general treatment strategy or a well-known
disease-drug association that is typically captured verbatim in a single summary entry,
with no enumeration or temporal precision required.
"""
