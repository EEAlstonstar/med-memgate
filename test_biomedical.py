#!/usr/bin/env python3
"""
med-memgate test script for the Biomedical dataset.
...
Usage:
  ...
  python test_biomedical.py --limit 10 --router-type llm --max-workers 2

  # Full run with vLLM router
  python test_biomedical.py \\
    --router-type vllm \\
    --router-base-url http://localhost:8000/v1 \\
    --router-model Qwen3-0.6B

  # Medical-enhanced BinaryRouter (no LLM router needed)
  python test_biomedical.py --router-type binary --limit 50
