...
运行示例：
  ...
  # 使用 vLLM 作为 Router
  python test_locomo_multi.py --limit 10 --router-type vllm \
...
Examples:
  ...
  python test_locomo_multi.py --limit 10 \\
    --router-type vllm \\
    --router-base-url http://localhost:8000/v1 \\
    --router-model Qwen3-4B-Thinking-2507

  # 使用默认 LLMRouter（不使用 OpenAI client）
  python test_locomo_multi.py --limit 10 --router-type llm
"""
    )
