#!/usr/bin/env python3
"""
并发版本的 med-memgate 测试脚本 - LongMemEval
...
运行示例：
  ...
  # 使用 vLLM 作为 Router
  python test_longmemeval_multi.py --limit 10 --router-type vllm \
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Test med-memgate on LongMemEval (Concurrent)",
...
Examples:
  ...
  python test_longmemeval_multi.py --limit 10 \\
    --router-type vllm \\
    --router-base-url http://localhost:8000/v1 \\
    --router-model Qwen3-4B-Thinking-2507

  # 使用默认 LLMRouter（不使用 OpenAI client）
  python test_longmemeval_multi.py --limit 10 --router-type llm
"""
    )
    ...
    if args.run_id:
        ...
        run_id = f"medmemgate_longmemeval_{timestamp}"
    ...
    print(f"\n{'='*60}")
    print("Testing med-memgate on LongMemEval")
    print(f"{'='*60}")
